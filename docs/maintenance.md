...
Cookie 的裸 `user_…` 可对应裸 subject 或 `提供方|user_…`（如 `auth0|`、`google-oauth2|`、`github|`）。Cookie ID 比较时提取后缀，完整 OAuth subject 继续用于上游身份校验及存储，不能因后缀相同就把不同提供方视为同一账号。V2 的本地 Cookie 校验失败返回 422 和固定错误码（`invalid_session_cookie` / `cookie_account_mismatch`），前端按白名单显示提示；上游请求/桌面凭证失败仍返回脱敏的 502。合成回归覆盖导入、重新授权、续期、身份不匹配及 Web/桌面提示。

网页 `me` 可返回同一账号的裸 `sub`，身份资料校验允许它匹配 JWT 的 `提供方|user_…`；此兼容仅用于上游身份资料，不剥离已签发 JWT 或持久化身份的提供方。桌面 `GetMe` 的 `authId`、网页/桌面 JWT 与保存的完整身份仍受原比较规则约束。Google 账号经用户明确授权的单次本地导入已实测通过，见 [Issue #2 补充验证](archive/2026-09-16-oauth-cookie.md#真实导入补充验证)；不据此推断 GitHub 实测、多设备续期或撤销行为。
...
- 上限优先使用本次反解，其次保留同账号、同套餐及同账期的历史；本次已知容量与历史冲突时放弃历史。Legacy 与 V2 都在成功快照写入时保留历史，跨账号估算不写回快照，避免反复传播。
...
- 三档均已耗尽且按量付费关闭、按量消费为 0 时，可用本周期消费辅助区分同套餐容量：仅匹配消费达到上限、超额不超过 1%（最低 2 美分容差）的已观测完整容量，且附近只能有一种综合容量。此规则是启发式估算，不保证所有请求的超额都在该范围内；不匹配、多个档位均接近或无消费时仍留空。结果标记 `limit_source=exhaustion`，显示 `≈`，不作为自身历史或其他账号的观测继续传播。
- Legacy 的 `pools.observe()` 按账号保存最近一次有效观测，部分解只贡献已知档位；每次成功刷新替换旧观测，删除账号清理观测，启动重建。观测无独立 TTL，仍受快照新旧程度影响；`plan_pools` 是观测计数。V2 仅使用当前授权查询可见账号，不能跨空间或越权借用观测。
- 补齐的档位标记 `limit_inferred` 及 `limit_source`（`history`、`plan`、`exhaustion` 或 V2 的 `reference`），不改变上游百分比。CLI 仅使用当前反解，不走历史或跨账号补齐。美元上限仍为估算。
