...
- 美元上限优先采用当前可解数据，再保留同账号、同套餐、同账期的历史上限；重新授权同一账号也会保留。可见同套餐账号的有效观测可按档位补齐缺项。历史已丢失时，账号管理者可在「编辑资料与标签 → 额度上限参考值」补填本账期已知金额；手动参考值不传播给其他账号，账期或套餐变化后失效，也不改变最后成功查询时间或剩余百分比。
