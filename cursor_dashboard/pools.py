...
# plan -> account -> latest own quota observation
...
def retain_own_limits(data: dict | None, previous: dict | None) -> dict | None:
    """Keep solved limits within the same account, plan and cycle.

    Called before saving a successful snapshot. Peer estimates are never persisted
    as own observations, so deleting their source takes effect immediately.
    """
